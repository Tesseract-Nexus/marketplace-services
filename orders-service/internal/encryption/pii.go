...
// PIIEncryptor handles encryption/decryption of PII data for orders service
...
func DefaultConfig() Config {
	return Config{
		...
		SecretName:            os.Getenv("ORDERS_PII_ENCRYPTION_SECRET"),
		LocalKey:              os.Getenv("ORDERS_PII_LOCAL_KEY"),
...
// MaskGSTIN masks a GSTIN for display (show first 2 and last 3)
func MaskGSTIN(gstin string) string {
	...
	return gstin[:2] + strings.Repeat("*", len(gstin)-5) + gstin[len(gstin)-3:]
}

// MaskVAT masks a VAT number for display
func MaskVAT(vat string) string {
	if vat == "" {
		return ""
	}
	if len(vat) <= 4 {
	...
	return vat[:2] + strings.Repeat("*", len(vat)-4) + vat[len(vat)-2:]
}

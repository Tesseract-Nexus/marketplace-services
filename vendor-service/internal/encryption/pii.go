...
// PIIEncryptor handles encryption/decryption of PII data for vendor service
...
func DefaultConfig() Config {
	return Config{
		...
		SecretName:            os.Getenv("VENDOR_PII_ENCRYPTION_SECRET"),
		LocalKey:              os.Getenv("VENDOR_PII_LOCAL_KEY"),
...
// MaskTaxID masks a tax identification number
func MaskTaxID(taxID string) string {
	...
	return taxID[:2] + strings.Repeat("*", len(taxID)-4) + taxID[len(taxID)-2:]
}
